...
const { baseUrl, programId, authorization } = require("./config.json");

const getEvents = (startDate, endDate) => {
	return fetch(
		`${baseUrl}/api/events.json?program=${programId}&startDate=${startDate}&endDate=${endDate}&skipPaging=true&order=lastUpdated:DESC`,
		{
...
const generateEventRows = component => {
	...
	component.state.events.forEach((event, index) => {
		...
		rows.push(
			<Tooltip
				key={index}
				title={<div className="tooltip-container">{tooltipText}</div>}
				placement="left"
				disableTriggerFocus={!tooltip}
				disableTriggerHover={!tooltip}
			>
				<div
					className={`row ${className}`}
					onClick={component.handleCheck(index, disableCheckbox)}
				>
...
const filter = (filteredByStatus, filteredByTestResult, events) => {
	...
	events.forEach(e => {
		...
		if (statusIndex === -1) {
			if (
				results.includes(e.dataValues[testResultIndex].value) &&
				status.includes("No Synced")
			) {
				e.showed = true;
			} else {
				e.showed = false;
			...
			if (
				results.includes(e.dataValues[testResultIndex].value) &&
				status.includes(e.dataValues[statusIndex].value)
			) {
				e.showed = true;
			} else {
				e.showed = false;
...
const checkEvents = (events, condition1, condition2) => {
	...
	return Promise.all(promises).then(() => {
		events.forEach(e => {
			if (e.checked) {
				let dataValueIndex = _.findIndex(
					eventsToCompare[e.event].dataValues,
					dv => {
				...
				if (dataValueIndex === -1) {
					...
					let status =
						eventsToCompare[e.event].dataValues[dataValueIndex].value;
...
const checkStatus = (events, type) => {
	...
	return fetch(
		`${baseUrl}/api/events.json?event=${checkedEvents}&skipPaging=true`,
		{
			credentials: "same-origin",
			headers: {
				Authorization: authorization
		...
		.then(json => {
			json.events.forEach(e => {
				if (type === "submit") {
					...
					if (
						dataValueIndex === -1 ||
						e.dataValues[dataValueIndex].value === "No Synced" ||
						e.dataValues[dataValueIndex].value === "Rejected"
					) {
